...
Generic classes
===============
A thermodynamic model in ``SigmaMu`` is composed of :class:`ThermoContribution` objects stacked on top of a :class:`StateDefinition` instance. The latter one interprets the numerical state vector as physical quantities, such as temperature, pressure and mole flows. Each contribution then builds on the already defined quantities to add new ones, until the model is complete.
